...
* 各値は公開URL用の安定 slug を持つ（`/season/{slug}`、`14 §4`）。slug は表示名と分離し、語彙の表示名を変えても URL を保てるようにする。日本語URL（例: `/season/春`）かローマ字/英語 slug（例: `spring`）かは技術設計（Step 9 ADR）で確定する。
...
* 各値は公開URL用の安定 slug を持つ（`/theme/{slug}`、`14 §4`）。slug は表示名と分離する。特に `移動・行き方` のような記号・複合語を含む値は、日本語URL（例: `/theme/移動・行き方`）かローマ字/英語 slug（例: `mobility`）かを技術設計（Step 9 ADR）で確定する。表示名を変更しても slug（=URL）は据え置けるようにする。
...
* slug の表記方針（日本語URL か ローマ字/英語 slug か）は Step 9 ADR で確定する。対象は Post / Place / Theme / Season の slug。特に `移動・行き方` のような記号・複合語を含むテーマ値の URL 表現を決める。slug は内部識別子（post_id 等）および表示名と分離し、URL の安定性を保つ方針は本書で固定済み（§4-1 / §4-2 / §4-3 / §4-4）。Tag の URL（`/tag/...`）はMVPでは設けない（タグは表示のみ・一覧ページなし。`14 §4`）。将来タグページを設ける場合のみ同 slug 方針を適用する。
